...
static int getLugarLibre(Contratacion* array, int len)// anda barbaro
{
    int retorno = -1;
    int i;
    if(array != NULL && len > 0)
...
Contratacion* contratacion_getByCuit(Contratacion* array, int len,char cuit[])//esto no lo entendi muy bien pero supongo que te retorna una contratacion apartir de un cuit que vos le mandas esto siempre y cuando el cuit coincidad con uno existente y halla algo en contrataciones
{
    Contratacion* retorno = NULL;
    contratacion_init(retorno,len);//supuestamente inizialisa todo en NULL
    int i;
    int lugarLibre;
    if(array != NULL && len > 0)//si el array que te mandaron de contrataciones tiene algo (por que es distinto de NULL)
    {
        ...
        {
            if(!array[i].isEmpty && strcmp(array[i].cuit,cuit) == 0)//si el array de contrataciones que te mandaron en la posicion que esta i no esta en empty y el cuit es igual al cuit que le mandaste entro.
            {
                lugarLibre = getLugarLibre(retorno,len);//esto no  lo entendi muy bien sorry...
                retorno[lugarLibre] = array[i];//esto no  lo entendi muy bien sorry...
            }
        }
    }
    return retorno;
...
int contratacion_alta(Contratacion* array,Pantalla* arrayPantalla, int len,int lenPantalla)//como el usuario antes de dar el alta de una contratacion tiene  que elejir la pantalla;se le pasa como parametro el array y el largo de la misma.
{
    ...
    {
        indice = getLugarLibre(array,len);//busca un lugar libre pero nose para que ? (estoy medio cansado igual)
...
int contratacion_modificar(Contratacion* array, int len, char cuit[],Pantalla* arrayPantallas, int lenPantallas)
{
    ...
    contratacionesDelCuit = contratacion_getByCuit(array,len,cuitIngresado);//las contrataciones que tiene el cliente
    ...
    //ACA ESTA ROMPIENDO ZARPADO!
    //CREO QUE ROMPE POR QUE NO RECONOCE QUE YO TENGO UNA CONTRATACION COMO PARA MODIFICAR ENTONCES AL NO PODER
    //MODIFICAR ALGO ROMPE.
    contratacionElegida = contratacion_getByPantalla(contratacionesDelCuit,len,idIngresado);//no entendi muy bien esto.... sorry
...
int contratacion_baja(Contratacion* array, int len, char cuit[], Pantalla* arrayPantallas, int lenPantallas)
{
    ...
    if(utn_getCuit(cuitIngresado,"Ingrese su cuit","Error ingrese un cuit valido!",2) == 0)//ya tengo el cuit..
    {
        //se listaran todas las pantallas de video que el cliente tiene contratadas mostrando todos sus campos.?????
    }
     utn_getNumero(&idIngresado,"ingrese el id de la pantalla","ingrese un id valido",0,10000,2);//tengo el id de la pantalla...
